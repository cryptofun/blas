...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("BlakeStar"))
        return false;
...
{
    // Convert BlakeStar:// to BlakeStar:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("BlakeStar://"))
    {
        uri.replace(0, 12, "BlakeStar:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "BlakeStar.lnk";
}
...
{
    return GetAutostartDir() / "BlakeStar.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("BlakeStar-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  BlakeStar-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("BlakeStar-Qt"));
